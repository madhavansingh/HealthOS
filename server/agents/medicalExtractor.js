 ...
 * Takes raw PDF text and returns structured metric array
 */
async function extractMetrics(reportText, reportType = 'unknown') {
  const promptTemplate = fs.readFileSync(promptPath, 'utf-8');
  const prompt = promptTemplate
    .replace('{reportText}', reportText.slice(0, 8000)) // Limit context
...
/**
 * Determine report icon/type for UI
 */
...
/**
 * Extracts structured medical metrics directly from scanned documents or images using Gemini Vision.
 */
...
- source: return "vision" for all of them.`;
  ...
  try {
    ...
    console.log(`[MedicalExtractor] Processing multimodal file ${originalName}...`);
    const result = await model.generateContent([prompt, filePart]);
    const responseText = result.response.text();
    console.log(`[MedicalExtractor] Raw response: ${responseText.slice(0, 300)}...`);
    ...
    console.error('[MedicalExtractor] Multimodal extraction failed:', err.message);
    throw err;
  }
}

module.exports = { extractMetrics, extractMetricsMultimodal, detectReportType, getReportCategory };
